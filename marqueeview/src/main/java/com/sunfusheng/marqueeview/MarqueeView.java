...
public class MarqueeView<T> extends ViewFlipper {
    ...
    private TextView createTextView(T marqueeItem) {
        ...
        } else if (marqueeItem instanceof IMarqueeImageItem) {
            message = ((IMarqueeImageItem) marqueeItem).marqueeMessage();
            imageRes = ((IMarqueeImageItem) marqueeItem).getImageRes();
        }
        if (0 != imageRes) {
            textView.setCompoundDrawablePadding(Utils.dip2px(getContext(),10));
            textView.setCompoundDrawablesRelativeWithIntrinsicBounds(imageRes, 0, 0, 0);
        }
