...
//        LogUtil.i(UIConstants.DEMO_TAG, "judgeMemberIsSelf-->member.getNumber="+member.getNumber()+",siteName->"+LoginCenter.getInstance().getSipAccountInfo().getSiteName());
        if (member.isSelf()
                ...
                //siteName是自己通过siteUri查询的数据
                || member.getNumber().equals(LoginCenter.getInstance().getSipAccountInfo().getSiteName())
                ) {
            return true;
        }
        return false;
    ...
    public int requestChairman(String chairmanPassword) {
        ...
        int result = -1;
        if (null != self) {
            result = this.tupConference.requestChairman(chairmanPassword, self.getMT());
            if (result != 0) {
